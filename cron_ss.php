 ...
 */
 ...
 require_once("/home/mangacom/public_html/wp-load.php");
 $cron_ss_crawl = new Cron_SS();
// $cron_ss_crawl->getMultiPageSSManga();
// $cron_ss_crawl->getAMangaSS("http://sstruyen.com/doc-truyen/do-you-remember-me/16053.html", "DO YOU REMEMBER ME???", 0);
// file_put_contents("/home/mangacom/D:3:a.2/data.txt", "abc123");
// $cron_ss_crawl->getDataFromUrlSSL("http://sstruyen.com/doc-truyen/index.php?ajax=ct&id=1446699&t=20180414162458", "/home/mangacom/D:3:a.2/data.txt");

class Cron_SS {

    function getImageComicVn($url, $path, $name, $index) {
        $content = $this->get_content_from_url($url);
        file_put_contents($path . "/site" . $index . ".html", $content);
        $DOM = new DOMDocument();
        $DOM->loadHTMLFile($path . "/site" . $index . ".html");

        $all_images = $DOM->getElementById("image-load")->item(0);
        $images = $all_images->getElementsByTagName("img");
        $data = "";
        for ($i = 0; $i < $images->length; $i++) {
            $image = $images->item($i);
            $src_image = $image->getAttribute("src");
            if ($src_image !== NULL && $src_image !== "") {
                $data .= '<img src ="' . $src_image . '" alt ="' . $name . '_' . $i . '"/>';
            }
        }
    ...
    function getDataFromUrlSSL($url_a_manga, $path) {
    ...
    function getAMangaA3Manga($url_a_manga, $index) {
        ...
        $slug_manga = $this->to_slug($title_manga);
...
//check update
            ...
            $latest_chap = $this->to_slug($this->removeSignImpossibleLinux($latest_chap));
            global $wpdb;
...
//update manga
                for ($i = 0; $i < $all_chap->length; $i++) {
                    ...
                    $update_chap_slug = $this->to_slug($this->removeSignImpossibleLinux($update_chap));
...
//add new
        if ($this->post_exists($title_manga) == null) {
            ...
            $folder_a_manga = $folder_mangas_sum . '/' . $this->to_slug($title_manga);
            if ($numPage !== -1) {
                for ($i = 1; $i < $numPage; $i++) {
...
//update manga
    ...
    function getNewMangaSSInAPage($url_a_manga, $is_home, $index) {
        ...
        $folder_slug = $this->to_slug($title_manga);
...
// $info_des = str_replace($info_des_ss_element->nodeValue, "", $info_des_element->nodeValue);
            ...
            $this->saveImageFromUrl($info_content_img, $folder_mangas_sum . '/' . $folder_slug . '/' . $this->to_slug($title_manga) . '.png');
...
//chapter file
        ...
        // $this->getDataFromUrlSSL($site, $folder_chap . '/data.txt');
        file_put_contents($folder_chap . '/data.txt', $site);
...
//set feature image
        $this->generate_Featured_Image($folder_a_manga . '/' . $this->to_slug($title_manga) . '.png', $crawl_post_id);
...
//add unique id for manga post
        ...
        $wp_manga_storage->local_remove_storage($folder_a_manga . '/' . $this->to_slug($title_manga) . '.png');
...
//create chapter
    ...
    private function post_exists($post_name, $post_type = 'wp-manga') {
        global $wpdb;
        ...
        if (!empty($post_name)) {
            $query .= " AND post_title = '" . $post_name . "'";
            $args[] = $post_name;
        }
        if (!empty($post_type)) {
